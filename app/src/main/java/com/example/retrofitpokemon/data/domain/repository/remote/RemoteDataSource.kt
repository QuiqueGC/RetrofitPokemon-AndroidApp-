...
import com.example.retrofitpokemon.data.domain.repository.remote.mapper.generation.GenerationMapper
...
class RemoteDataSource(private val apiCallService: ApiCallService) : DataSource {
    ...
    override fun getGeneration(idGeneration: Int): Flow<BaseResponse<GenerationModel>> =
        flow {
            val apiResult = apiCallService.getGeneration(idGeneration)
            if (apiResult is BaseResponse.Success) {
                emit(BaseResponse.Success(GenerationMapper().fromResponse(apiResult.data)))
            } else if (apiResult is BaseResponse.Error) {
                emit(BaseResponse.Error(apiResult.error))
            }
